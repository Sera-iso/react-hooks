...
Use an arrow function to lazy initialize a useState.
In this way useState is called with a function that returns the initial state (rather than simply passing the initial state).
Remember: the only time React needs the initial state is initially. Meaning, it only really needs the initial state on the first render. But because our function body runs every time there's a re-render of our component, we end up running that code on every render, even if its value is not used or needed.
...
From: https://kentcdodds.com/blog/use-state-lazy-initialization-and-function-updates
